...
public class AllInOneTester {
	...
	public static void main(String[] args) {
		...
		Collection<String> data = watchBrandsList.stream().filter((value) -> value.startsWith("T"))
		...
		List<String> pan = panList.stream().filter((temp) -> temp.startsWith("A")).collect(Collectors.toList());
		...
		Collection<String> usnList = Arrays.asList("1SJ16EC036", "1SJ16CS060", "1SJ16CV065", "1SJ16CS090",
				"1SJ16CS046");
		Collection<String> usn = usnList.stream().filter((d) -> d.contains("CS")).collect(Collectors.toList());
